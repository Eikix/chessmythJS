import convertBoardOptionsToInitialBoard from '../utils/helpers/convertStringToBoard';
import pieceTypeToMana from '../utils/pieces/pieceTypeToMana';
import processMoves from '../utils/moves/processMoves';
...
class Board {
	...
	constructor(boardOptions, piecesAndMoves) {
		...
		this.board = convertBoardOptionsToInitialBoard(boardOptions, piecesAndMoves);
		this.manaCount = {
			b: 0,
			w: 0
		};
	...
	getPossibleMoves(coord, playerColor) {
		...
		if (pieceTypeOnCoord && this.#piecesAndMoves.hasOwnProperty(pieceTypeOnCoord)) {
			return processMoves(
				this.#dimension,
				coord,
	...
	move(from, to, playerColor) {
		...
		if (targetMove) {
			...
			if (this.board[from].piece.type === 'k' && this.board[from].piece.color === playerColor) {
				if (playerColor === 'w') this.wKing = to;
				if (playerColor === 'b') this.bKing = to;
			}

			// Checks if the piece still has special move charges
			if (targetMove.isSpecialMove && this.board[from].piece.specialMoveCharges === 0) {
				return false;
			}
			...
			if (this.board[to].piece) {
				this.casualties[otherPlayerColor].push(this.board[to].piece.type);
				this.manaCount[playerColor] += this.pieceTypeToMana[this.board[to].piece.type];
			}
