import Board from './boards/CMBoard';
import exitCheckMoveList from './utils/helpers/exitCheckMoveList';
import isCheck from './utils/helpers/isCheck';
import allPossibleMovesOnBoard from './utils/moves/allPossibleMovesOnBoard';
...
class Chess extends Board {
	...
	getLegalMoves(coord) {
		...
		if (DEBUG) console.log('exitCheckMoveList returned value: ', allowedMoves);
		const moves = this.getPossibleMoves(coord, playerColor);
		if (DEBUG) console.log('Allowed Moves without taking in account Check: ', moves);
		moves.forEach((move) => {
			...
			allowedMoves.forEach((legalMove) => {
				if (isEqual(legalMove, movement)) legalMoves.push(movement);
			});
		});
		if (DEBUG) console.log('Legal Moves to be returned from getLegalMoves : ', legalMoves);
		return legalMoves;
	}
	...
	moveFromTo(from, to) {
		...
		if (this.board[from]?.piece?.color === playerColor) {
			...
			legalMoves.forEach((legalMove) => {
				if (isEqual(legalMove, desiredMove)) completeMove = this.move(from, to, playerColor);
			});
			if (completeMove) {
				this.#turn += 1;
				...
				const nextPlayerKing = nextPlayerColor === 'w' ? this.wKing : this.bKing;
