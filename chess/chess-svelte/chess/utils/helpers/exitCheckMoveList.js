import allPossibleMovesOnBoard from '../moves/allPossibleMovesOnBoard';

export default function exitCheckMoveList(playerKing, dimension, mobility, playerColor, board) {
	const exitCheckMovesArray = [];
	const allMoves = allPossibleMovesOnBoard(dimension, mobility, playerColor, board, true);
	...
	for (let from of fromArray) {
		...
		for (let move of allMoves[from].moves) {
			...
			if (move.isSpecialMove && hypotheticBoard[from].piece.specialMoveCharges === 0) {
				continue;
			}
